...
// We track it ourselves so we can fully cancel the refresh tick while
// hidden — keeping a setInterval alive (even one that bails) prevents
// WebView2 from settling into a true idle, which we saw as a slow CPU
// climb across open/close cycles.
...
function startRefresh() {
  ...
  // 30s cadence: minute-level "next run in Xm" precision is plenty.
...
listen('window-visibility', (e) => {
  windowVisible = !!e.payload;
  if (windowVisible) startRefresh(); else stopRefresh();
});
